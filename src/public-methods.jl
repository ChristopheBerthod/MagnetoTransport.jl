...
Particle number for chemical potential `μ` and temperature `T` (energy units). `HoN0` is a
[`HilbertTransform`](@ref) object holding the Hilbert transform of the density of states.
`Σ(::Real)` is a function giving the self-energy. `tol` sets the relative tolerance of the
quadratures (default `1e-8`).
"""
...
Particle number for chemical potential `μ` and temperature `T` (energy units) in the
noninteracting case (``\\Sigma(\\varepsilon)=-i0^+``). `HoN0` is a
[`HilbertTransform`](@ref) object holding the Hilbert transform of the density of states.
`tol` sets the relative tolerance of the quadratures (default `1e-8`).
"""
...
Chemical potential for density `n` and temperature `T` (energy units). `HoN0` is a
[`HilbertTransform`](@ref) object holding the Hilbert transform of the density of states.
`Σ(::Real)` is a function giving the self-energy. `μ0` is an optional initial estimate.
`tol` sets the relative tolerance of the quadratures and root finding (default `1e-8`).
"""
...
Chemical potential for density `n` and temperature `T` (energy units) in the noninteracting
case (``\\Sigma(\\varepsilon)=-i0^+``). `HoN0` is a [`HilbertTransform`](@ref) object
holding the Hilbert transform of the density of states. `μ0` is an optional initial
estimate. `tol` sets the relative tolerance of the quadratures and root finding (default
`1e-8`).
"""
...
Normal conductivity in units of ``e^2/h`` for density `n` and temperature `T` (energy
units). `L2oPhi0` is a [`LorentzTransform`](@ref) object holding the ``L^2`` transform of
the transport function divided by ``(e/\\hbar)^2``. `Σ(::Real)` is a function giving the
self-energy. `HoN0` is a [`HilbertTransform`](@ref) object holding the Hilbert transform of
the density of states. `εmin` and `εmax` are optional limits of integration at finite-``T``
(default `-Inf` and `+Inf`). `μ0` is an optional initial estimate for the chemical
potential. `tol` sets the relative tolerance of the quadratures and root finding (default
`1e-8`).
"""
...
Normal conductivity in units of ``e^2/h`` for chemical potential `μ` and temperature `T`
(energy units). `L2oPhi0` is a [`LorentzTransform`](@ref) object holding the ``L^2``
transform of the transport function divided by ``(e/\\hbar)^2``. `Σ(::Real)` is a function
giving the self-energy. `εmin` and `εmax` are optional limits of integration at finite-``T``
(default `-Inf` and `+Inf`). `tol` sets the relative tolerance of the quadrature (default
`1e-8`). 
"""
...
Hall conductivity divided by ``B|e|^3/h^2`` for density `n` and temperature `T` (energy
units). `L3oPhi1` is a [`LorentzTransform`](@ref) object holding the ``L^3`` transform of
the transport function divided by ``(|e|/\\hbar)^3``. `Σ(::Real)` is a function giving the
self-energy. `HoN0` is a [`HilbertTransform`](@ref) object holding the Hilbert transform of
the density of states. `εmin` and `εmax` are optional limits of integration at finite-``T``
(default `-Inf` and `+Inf`). `μ0` is an optional initial estimate for the chemical
potential. `tol` sets the relative tolerance of the quadratures and root finding (default
...
Hall conductivity divided by ``B|e|^3/h^2`` for chemical potential `μ` and temperature `T`
(energy units). `L3oPhi1` is a [`LorentzTransform`](@ref) object holding the ``L^3``
transform of the transport function divided by ``(|e|/\\hbar)^3``. `Σ(::Real)` is a function
giving the self-energy. `εmin` and `εmax` are optional limits of integration at finite-``T``
(default `-Inf` and `+Inf`). `tol` sets the relative tolerance of the quadrature (default
`1e-8`).
"""
...
Hall constant multiplied by ``|e|`` for density `n` and temperature `T` (energy units).
`L2oPhi01`, `L2oPhi02`, and `L3oPhi1` are [`LorentzTransform`](@ref) objects holding the
``L^2`` and ``L^3`` transforms of the transport functions. `Σ(::Real)` is a function giving
the self-energy. `HoN0` is a [`HilbertTransform`](@ref) object holding the Hilbert transform
of the density of states. `εmin` and `εmax` are optional limits of integration at
finite-``T`` (default `-Inf` and `+Inf`). `μ0` is an optional initial estimate for the
chemical potential. `tol` sets the relative tolerance of the quadratures and root finding
(default `1e-8`).
"""
...
Hall constant multiplied by ``|e|`` for chemical potential `μ` and temperature `T` (energy
units). `L2oPhi01`, `L2oPhi02`, and `L3oPhi1` are [`LorentzTransform`](@ref) objects holding
the ``L^2`` and ``L^3`` transforms of the transport functions. `Σ(::Real)` is a function
giving the self-energy. `εmin` and `εmax` are optional limits of integration at finite-``T``
(default `-Inf` and `+Inf`). `tol` sets the relative tolerance of the quadratures (default
`1e-8`).
"""
